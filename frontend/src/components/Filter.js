...
function Filter() {
  ...
  const sendOneWayRequest = async () =>{
    const res = await axios
    .get(`http://localhost:5000/?originCity=${originCity}&destinationCity=${destinationCity}&departureDate=${departureDate}`)
    .catch(err => console.log(err));

    const data = await res.data;
    console.log(originCity,destinationCity,departureDate)
    console.log(data)
    return data;
  };
  ...
  const sendReturnRequest = async () =>{
    const res = await axios
    .get(`http://localhost:5000/?originCity=${originCity}&destinationCity=${destinationCity}&departureDate=${departureDate}&returnDate=${returnDate}`)
    .catch(err => console.log(err));
  ...
  const handleFilter = async () => {
    let result = await sendOneWayRequest((data) => {
      if (
        data &&
  ...
  const returnFilter = async () => {
    let result = await sendReturnRequest((data) => {
      if (
        data &&
        data.price <= priceRange 
      ) 
